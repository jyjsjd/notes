...
ConcurrentHashMap使用了一个**Node<K,V>[] table**来存储 Node，用 key 的 hashCode 来寻找记录的存储index；用**链表**解决冲突，当链表的长度大于 8 的时候会将链表转化为**红黑树**，从而将查找的复杂度从O(N)降到了O(lgN)。
...
ConcurrentHashMap 维护 `baseCount` 来表示当前的记录数量，在 put 操作和 remove 操作的时候回通过调用方法 addCount 来更新它。
